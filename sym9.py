...
# Новый класс для потока обработки камеры
class CameraThread(QThread):
    ...
    def run(self):
        # Загружаем модель YOLO в этом потоке
        try:
            model_path = "./runs/detect/train3/weights/best.pt"
            self.model = YOLO(model_path)
        except Exception as e:
            self.error_occurred.emit(f"Ошибка загрузки модели: {e}")
            return
        ...
        try:
            ...
            while self.running:
                ...
                results = self.model.predict(frame, imgsz=640, conf=0.085, verbose=False)
...
class QualityControlGUI(QMainWindow):
    def __init__(self):
        ...
        # Заменяем таймер камеры на поток камеры
    ...
    ### CAMERA: инициализация камеры через поток
    def init_camera(self):
        if not self.camera_thread.isRunning():
            self.camera_thread.start()

    ### CAMERA: обновление кадра из потока
    ...
    ### CAMERA: обработка ошибок из потока
    def handle_camera_error(self, error_msg):
        self.cam_image.setText(error_msg)
        self.cam_image.setAlignment(Qt.AlignCenter)
